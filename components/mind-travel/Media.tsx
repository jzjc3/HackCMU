...
export function DictationButton({onText,onError,disabled=false}:{onText:(s:string)=>void;onError:(s:string)=>void;disabled?:boolean}){
 ...
 recorder.onerror=()=>{if(alive.current)onError('Recording failed. You can still type your experience.');recorder.stop()};
 ...
 }catch{tracks.current?.getTracks().forEach(t=>t.stop());if(alive.current)onError('Microphone unavailable. Allow microphone access or type your experience.')}};
