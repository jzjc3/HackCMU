...
type Resources={
  ...
  pendingAssistant:PendingAssistant[]; unansweredUserTurns:number; capturedUserText:string[]; categorizationInFlight:boolean;
...
const freshResources=():Resources=>({
  ...
  pendingAssistant:[],unansweredUserTurns:0,capturedUserText:[],categorizationInFlight:false,
});
      ...
      ws.onmessage=async e=>{
        ...
        try{
          ...
          if(type==='conversation.item.input_audio_transcription.completed'&&event.transcript?.trim()){
            ...
            if(!repeatedFallback&&(!key||!r.seenInputItems.has(key))){if(key)r.seenInputItems.add(key);else r.lastUserFallback={text,at:Date.now()};r.capturedUserText.push(text);callbacks.current.onMessage('me',text);r.unansweredUserTurns++;flushAssistant()}
          }
          ...
          if(type==='response.function_call_arguments.done'){
            ...
            if(!r.seenToolCalls.has(callKey)){
              ...
              try{
                if(event.name==='categorize_experiences'){
                  if(r.categorizationInFlight)output={result:'Categorization is already in progress. Wait for its result before continuing.'};
                    ...
                    const capturedCount=r.capturedUserText.length,text=r.capturedUserText.slice(0,capturedCount).join('\n').trim();if(!text)throw new Error('No completed user transcript is available yet.');r.categorizationInFlight=true;
                    try{
                      const classification=await clientApi.extract(text,world.dims,{recent:world.memories.slice(-8).map(m=>({text:m.text,dims:m.dims}))});
                      const proposals=classification.items.filter(x=>x.dims.length&&x.dims.every(d=>ids.has(d))).slice(0,6);
                      if(proposals.length)callbacks.current.onProposals(proposals);r.capturedUserText.splice(0,capturedCount);
              ...
              }catch(toolError){const message=toolError instanceof Error?toolError.message:'The classifier is unavailable.';setError(message);output={result:`Classification failed: ${message}. Ask the user to retry or use typed capture.`}}
          ...
          if(type==='response.done'){const doneId=id||r.activeResponseId||'pending';r.completedResponses.add(doneId);if(r.activeResponseId===doneId)r.activeResponseId=undefined;maybeContinue(doneId)}
