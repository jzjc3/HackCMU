...
const reports=[];
for(const mode of ['v1-default','v6-transcribe']){
  ...
  await new Promise((resolve,reject)=>{
    ...
    ws.onopen=()=>send({type:'session.update',session:{voice:'eve',instructions:'You are Mind Travel. When the user describes an experience and asks to review it, call categorize_experiences. Do not invent experiences.',turn_detection:{type:'server_vad'},audio:{input:{format:{type:'audio/pcm',rate:24000},...(mode==='v6-transcribe'?{transcription:{model:'grok-transcribe'}}:{})},output:{format:{type:'audio/pcm',rate:24000}}},tools:[{type:'function',name:'categorize_experiences',description:'Prepare the spoken experience for review.',parameters:{type:'object',properties:{},required:[],additionalProperties:false}}]}});
